...
def fetch_data_from_notion_analyse():
    ...
    for result in data['results']:
        ...
        num = properties['num']['rich_text'][0]['text']['content']

        rows.append([num,environnement, politique, agriculture, economie,education,transport,religion,sante,travail,sport,justice,loisir,social,technologie])
...
def graphs():
    ...
    rows = fetch_data_from_notion_analyse()

   

    # Convertir les données en DataFrame
    data = pd.DataFrame(rows, columns=['num','environnement', 'politique', 'agriculture', 'economie','education','transport','religion','sante','travail','sport','justice','loisir','social','technologie'])
    ...
    data[['theme_major1', 'theme_major2']] = data.apply(find_major_themes, axis=1, result_type='expand')

    # Afficher les trois premières lignes du DataFrame avec les nouvelles colonnes
    print("Les trois premières lignes du DataFrame avec les thèmes majoritaires :")
    print(data.head(3))
    ...
    by_theme_major1 = data.groupby(by='theme_major1').size()
    ...
    by_theme_major2 = data.groupby(by='theme_major2').size()
...
def save():
    ...
    notion_data = {'en_tant_que': en_tant_que, 'jaimerais': jaimerais, 'afin_de_parce_que': afin_de_parce_que}
...
def generate_wordcloud():
    ...
    # Convertir les données en DataFrame
    df = pd.DataFrame(rows, columns=['en_tant_que', 'jaimerais', 'afin_de_parce_que'])
